...
class GraphNeuralNetworkIndex(HedonicIndex):
    """Graph neural network house price index."""

    @staticmethod
    def get_model() -> type[BaseHousePriceModel]:
        """Get a graph neural network house price model."""
...
class GraphNeuralNetworkModel(NeuralNetworkModel):
    """Graph neural network house price model."""
    ...
    def _model_with_coefficients(
        ...
        **kwargs: Any,
        ...
        """Fit model and populate coefficients to produce index."""
        model = GraphNeuralAVM(**kwargs["init"]).fit(X, y, **kwargs["fit"])
        kwargs = self._inject_dataloader_kwargs(model, kwargs)
        if estimator == "residual":
            return self._model_residual(model, **kwargs["predict"])
        elif estimator == "attributional":
            return self._model_attributional(model, **kwargs["predict"])
        raise ValueError

    def _inject_dataloader_kwargs(self, model: NeuralAVM, kwargs: Any) -> Any:
        date_col = kwargs["predict"]["date"]
        latitude_col = "latitude"
        longitude_col = "longitude"
        explain_df, _ = self._create_explainer_input_data(
            model,
            kwargs["predict"]["input_df"],
            date_col,
            kwargs["predict"]["data_pipeline"],
        )
        explain_df[date_col] = pd.to_datetime(explain_df[date_col], format="%Y-%m-%d")
        for key in [date_col, latitude_col, longitude_col]:
            kwargs["predict"]["dataloader_kwargs"][key] = explain_df[key]
        return kwargs
    ...
    def fit(  # type: ignore
        ...
        conv_dims: Optional[Union[int, list[int]]] = None,
        dense_dims: Optional[Union[int, list[int]]] = None,
        emb_size: int = 5,
        ...
        Args:
            ...
            cov_dims (Optional[Union[int, list[int]]], optional):
                Convolutional layer sizes.
                If None, defaults to [32, 32].
            dense_dims (Optional[Union[int, list[int]]], optional): Dense
                layer sizes.
        ...
        geo_cols = {"latitude_col": "latitude", "longitude_col": "longitude"}
        input_cols = list(set(train_df.columns).union(set(geo_cols.values())))
        ...
        config = {
            ...
            "fit": {
                ...
                "dataloader_kwargs": {
                    "k": k,
                    "num_neighbors": num_neighbors,
                    "sale_date": self.hpi_df[date],
                    "latitude": self.hpi_df[geo_cols["latitude_col"]],
                    "longitude": self.hpi_df[geo_cols["longitude_col"]],
                },
            },
            "predict": {
                "input_df": self.hpi_df[input_cols],
                "date": date,
                "data_pipeline": data_pipeline,
                "min_pred_epoch": min_pred_epoch,
                "quantile": quantile,
